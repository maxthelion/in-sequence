...
struct PhraseWorkspaceView: View {
    ...
    private func handleSingleTap(on phraseID: UUID, trackID: UUID) {
        session.selectPhrase(id: phraseID)
        session.selectTrack(id: trackID)
                ...
                HStack(spacing: gridSpacing) {
                    ...
                    ForEach(Array(visibleTrackSlots.enumerated()), id: \.offset) { _, track in
                        Group {
                            if let track {
                                Button {
                                    session.selectTrack(id: track.id)
                                } label: {
                ...
                ForEach(Array(phrases.enumerated()), id: \.element.id) { index, phrase in
                    HStack(alignment: .top, spacing: gridSpacing) {
                        PhraseMatrixPhraseCell(
                            ...
                            session.selectPhrase(id: phrase.id)
                        }
    ...
    private func toggleBooleanCell(phraseID: UUID, trackID: UUID) {
        ...
        let updatedCell: PhraseCell

        switch currentCell {
        case .inheritDefault, .curve, .single:
            updatedCell = .single(toggledValue)
        case let .bars(values):
            updatedCell = .bars(Array(repeating: toggledValue, count: values.count))
        case let .steps(values):
            updatedCell = .steps(Array(repeating: toggledValue, count: values.count))
        }

        session.setPhraseCell(updatedCell, layerID: selectedLayer.id, trackIDs: [trackID], phraseID: phraseID)
    }
}
