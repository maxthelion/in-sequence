...
0. **Review the perform/setup build-out** (now 4/7 landed: global mode,
   ...
   - Tracks-edit card height (capture review F1): shrink to content,
     ...
     decides.)
1. **WhatsApp quarantine — final step.** Spot-check chats, then
   `rm -rf ~/whatsapp-media-quarantine` frees 82GB. Disk runs
   one-build-at-a-time until then (~3.8Gi free).
2. **Restart coreaudiod** in a real Terminal (`sudo killall
