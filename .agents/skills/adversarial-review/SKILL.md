...
Canonical mindset, hunt checklist, and report format live in the **`adversarial-reviewer`** subagent at `.Codex/agents/adversarial-reviewer.md`. The per-invocation scaffold (base ref, HEAD, plan, diff, commit log) is at `./reviewer-prompt.md`. Dispatch passes the filled-in scaffold to the agent.
