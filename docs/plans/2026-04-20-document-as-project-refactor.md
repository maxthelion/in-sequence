...
**Depends on:** current `main` after the post-bridge cleanup and destination-editor cleanup (`60fa69b`, `0df85d7`). This is a high-churn rename that touches almost every `document.model` call site, so it should run on a quiet tree and land **before** `ui-organisation-and-track-source-split` and `phrase-workspace-split`, not alongside them.
...
- **Redesigning `selectedTrack` / `selectedPhrase` accessor semantics.** The current setters silently bootstrap from empty and rebind selection when `id` changes — a real contract problem flagged in review. But fixing it touches 20+ UI call sites that use `$document.model.selectedTrack.foo` bindings, and the right replacement (`updateSelectedTrack(_:)` vs `replaceSelectedTrack(_:)`) needs thought about SwiftUI binding semantics. Separate plan.
...
Sources/
  Document/
    Project.swift                                 # RENAMED from SeqAIDocumentModel.swift; shrunk to ~200 LOC
    ...
    Project+TrackSources.swift                    # RENAMED from SeqAIDocumentModel+TrackSources.swift
    StepSequenceTrack.swift                       # NEW — extracted from SeqAIDocumentModel.swift:865-1050
    TrackType.swift                               # NEW — extracted from SeqAIDocumentModel.swift:1052-1078
    TrackMixSettings.swift                        # NEW — extracted from SeqAIDocumentModel.swift:1080-1094
...
## Task 1: Rename `SeqAIDocumentModel` → `Project` and `SeqAIDocument.model` → `.project`
...
**Files:**
- Rename: `Sources/Document/SeqAIDocumentModel.swift` → `Sources/Document/Project.swift` (via `git mv`)
- Rename: `Sources/Document/SeqAIDocumentModel+TrackSources.swift` → `Sources/Document/Project+TrackSources.swift` (via `git mv`)
- Modify: `Sources/Document/SeqAIDocument.swift` — change `var model: SeqAIDocumentModel` to `var project: Project`; update `init(model:)` → `init(project:)`; adjust `init(configuration:)` decode target and `fileWrapper(configuration:)` encode source.
- Modify: every call site in `Sources/`, `Tests/` that references `SeqAIDocumentModel` (type) or `document.model` / `.model` (property) where the LHS is a `SeqAIDocument` / `$document`.
...
**Subtleties:**
- `.model` is too common a token to blind-sed across the repo. Scope the replacement to the patterns `document.model`, `$document.model`, `SeqAIDocument(model:`, `.model =`, `.model.` when the left-hand identifier's type is `SeqAIDocument`. Verify by compiling after each batch.
...
- [ ] `git mv Sources/Document/SeqAIDocumentModel.swift Sources/Document/Project.swift`
- [ ] `git mv Sources/Document/SeqAIDocumentModel+TrackSources.swift Sources/Document/Project+TrackSources.swift`
- [ ] Inside `Project.swift`: `struct SeqAIDocumentModel` → `struct Project`, `static let empty = SeqAIDocumentModel(...)` → `static let empty = Project(...)`, `Self.defaultDestination` / `Self.defaultPatternBanks` / `Self.syncPatternBanks` references unchanged (they use `Self`).
- [ ] Inside `Project+TrackSources.swift`: `extension SeqAIDocumentModel` → `extension Project`.
- [ ] Inside `SeqAIDocument.swift`: `var model: SeqAIDocumentModel` → `var project: Project`; `init(model: SeqAIDocumentModel = .empty)` → `init(project: Project = .empty)`; `self.model = ...` → `self.project = ...` (two sites: default init, decode init); `try encoder.encode(model)` → `try encoder.encode(project)`.
- [ ] Grep-and-replace `SeqAIDocumentModel` → `Project` across `Sources/` and `Tests/`. Verify each hit is the type (not a substring of something else).
- [ ] Grep-and-replace `document.model` → `document.project` and `$document.model` → `$document.project` — handle carefully; don't touch `.model` on non-SeqAIDocument values.
- [ ] Regenerate xcodeproj: `xcodegen generate`.
- [ ] `xcodebuild -scheme SequencerAI test` — green.
- [ ] Update wiki pages: `grep -rln 'SeqAIDocumentModel\|document\.model' wiki/ docs/` then edit each to use `Project` / `document.project`.
- [ ] Commit: `refactor(document): rename SeqAIDocumentModel to Project and SeqAIDocument.model to .project`
...
**Subtleties:**
- `StepSequenceTrack.defaultDestination` references `SeqAIDocumentModel.defaultDestination(for:)` via an explicit `SeqAIDocumentModel.defaultDestination(...)` call in the initializer. After Task 1 that is `Project.defaultDestination(...)` — double-check the reference compiles after the extraction.
...
- `grep -rn 'SeqAIDocumentModel' Sources/ Tests/ wiki/ docs/` — zero hits.
...
|---|---|
| Domain type is named `Project`, not `SeqAIDocumentModel` | 1 |
...
- **Coordination with the UI plans.** `ui-organisation-and-track-source-split` and `phrase-workspace-split` both touch `document.model` call sites. This rename should land first so those plans build directly against `Project` / `document.project` instead of creating another churn layer to rebase later.
