...
| 7 | Capture/Discard are visible but disabled when Perform is off. | Partial | `PhraseWorkspaceView` renders Capture/Discard in the phrase shell. `PhrasePerformActionAvailabilityPresentation` keeps both disabled while Perform is off; while Perform is on, Discard is available for the live copy and Capture additionally requires dirty changes. This still needs visual review to prove disabled/off-state presentation is clear. |
