...
- Verified enough to keep building: phrase overlay state, capture/discard model,
  ...
  Apply writes, and quantized, length-limited phrase-layer latches for Mute,
  Fill, and Pattern across one or more bars. Engine-visible playback snapshot
...
- Not yet verified enough to ship as the full feature: visual match to the V3
  wireframes, latch timing semantics for scalar layers if those are meant to
  schedule rather than land immediately, scene
...
| 7 | Capture/Discard are visible but disabled when Perform is off. | Partial | `PhraseWorkspaceView` renders Capture/Discard in the phrase shell. `PhrasePerformActionAvailabilityPresentation` keeps both disabled while Perform is off; entering Perform creates a clean live phrase copy, making Capture and Discard available even before edits are staged. This still needs visual review to prove disabled/off-state presentation is clear. |
