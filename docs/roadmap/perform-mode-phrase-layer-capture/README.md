---
status: exploratory
stage: prototype
priority: high
blocked_by: []
---

# Perform Mode, Phrases, Layers, And Capture

Exploratory wireframes for a richer perform-mode information architecture:

- phrase-aware transport and cueing;
- layer changes across selected tracks;
- layer-perform matrix across tracks;
- track/drum-group perform detail;
- capture clip and capture phrase flows.

## Artifacts

- [reasoning.md](reasoning.md)
- [reasoning-v2.md](reasoning-v2.md)
- [prototypes/01-perform-home-and-phrase-cue.html](prototypes/01-perform-home-and-phrase-cue.html)
- [prototypes/02-layer-bulk-and-matrix.html](prototypes/02-layer-bulk-and-matrix.html)
- [prototypes/03-track-drum-capture-phrase.html](prototypes/03-track-drum-capture-phrase.html)
- [prototypes/04-phrase-matrix-system-v2.html](prototypes/04-phrase-matrix-system-v2.html)

## Feedback

- [feedback/2026-06-17-perform-phrase-time-values.md](feedback/2026-06-17-perform-phrase-time-values.md)
- [feedback/2026-06-17-defer-performance-groups.md](feedback/2026-06-17-defer-performance-groups.md)
- [feedback/2026-06-17-phrase-layer-cell-shape.md](feedback/2026-06-17-phrase-layer-cell-shape.md)
- [feedback/2026-06-17-phrase-scene-macro-events.md](feedback/2026-06-17-phrase-scene-macro-events.md)
- [feedback/2026-06-17-phrase-cell-edit-tuple.md](feedback/2026-06-17-phrase-cell-edit-tuple.md)

## Reference Captures Used

- `.meta/multipass/runtime/loops/project/observe/qa-surface-coverage/01-phrase.png`
- `.meta/multipass/runtime/loops/project/observe/qa-surface-coverage/03-tracks-perform.png`
- `.meta/multipass/runtime/loops/project/observe/qa-surface-coverage/11-phrase-perform-overlay.png`
- `.meta/multipass/runtime/loops/project/observe/qa-surface-coverage/14-tracks-perform-layer-selector.png`
- `.meta/multipass/runtime/loops/project/observe/qa-surface-coverage/22-track-history-tab.png`
- `.meta/multipass/runtime/loops/project/observe/qa-surface-coverage/29-drum-kit-matrix.png`
