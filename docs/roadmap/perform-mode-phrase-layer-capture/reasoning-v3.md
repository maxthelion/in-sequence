...
- Phrase editing now has two explicit modes. Edit changes the phrase baseline directly and does not show Capture or Discard. Perform creates a temporary live copy/overlay; Capture and Discard appear only there, alongside a dirty summary of the cells that would be printed.
- Phrase automation is separate from the performance overlay. A phrase-level Automation On/Off control decides whether scheduled per-bar, ramp, or continuous values are read while playing. Perform changes override/audition on top of that, but do not implicitly become automation.
