...
- Global Apply is a broader scoped action mode inside the phrase. It now follows the spirit of the current layer picker: an 8-column matrix of layer/value actions, a compact scope count in the top bar, and a track-selector overlay that also uses 8-column cells so MIDI surfaces can map to it quickly.
