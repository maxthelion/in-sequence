...
| RT-6 | Open | concurrency B6 | Sample file open/cache work must not run on the tick path under `lifecycleLock`. | Sample cache is warmed or asynchronously resolved; dense sample playback does not perform file IO on the tick queue. |
...
| RT-10 | Open | red-team/render harness briefs | The app needs a repeatable realtime evidence gate, not only unit tests. | A short deterministic render/churn lane runs dense sample playback, UI movement, start/stop, and routing changes, then records pass/fail evidence. |
