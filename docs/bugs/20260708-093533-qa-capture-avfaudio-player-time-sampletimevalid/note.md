...
Status: RESOLVED 16ac1dea; fresh qa-surface-coverage run 20260708-090903-in-sequence-qa-surface-coverage-main-923a889d absorbed all 79 PNG rows without reproducing the AVFAudio abort. Rows 29c, 29d, and 29f needed narrow status-timeout backfills after the full run, so the AVFAudio abort is fixed, while drum-kit command ack latency is tracked separately in docs/bugs/20260708-094500-qa-drum-kit-command-ack-latency/.
